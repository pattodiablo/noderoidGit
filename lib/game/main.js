...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		this.loadLevel( LevelNivelUno );
	 	...
	 	socket.on('entityMoved', function(posx,posy,nombre,animacion){
			...
			if(netPlayerArray){
				...
				for(var i in netPlayerArray){
					if(nombre==netPlayerArray[i].netName){
						...
						switch(animacion){
							case"run":
							netPlayerArray[i].currentAnim=netPlayerArray[i].anim.run;
							break;
		...
		socket.on(	'populating', function(allEntities){
   			...
   			for(var i in allEntities){
   				...
   				if(allEntities[i]!=socket.io.engine.id){
   					ig.game.spawnEntity("EntitynetPlayer",200,100,allEntities[i]);
